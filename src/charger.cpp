...
void charger_state_machine(dc_bus_t *bus, battery_conf_t *bat_conf, battery_state_t *bat_state, float voltage, float current)
{
    ...
    if (bus->dis_allowed == true
        && voltage < bat_conf->voltage_load_disconnect - current * bus->dis_droop_res)
    {
    ...
    if (voltage >= bat_conf->voltage_load_reconnect - current * bus->dis_droop_res
    ...
    case CHG_STATE_IDLE: {
        ...
        {
            bus->chg_voltage_target = bat_conf->voltage_topping + bat_conf->temperature_compensation * (bat_state->temperature - 25);
    ...
    case CHG_STATE_BULK: {
        ...
        if (voltage > bus->chg_voltage_target - current * bus->chg_droop_res) {
    ...
    case CHG_STATE_TOPPING: {
            ...
            || (time(NULL) - bat_state->time_state_changed) > bat_conf->time_limit_topping)
        {
            ...
            /*if (bat_conf->equalization_enabled) {
                ...
                bus->chg_voltage_target = bat_conf->voltage_trickle + bat_conf->temperature_compensation * (bat_state->temperature - 25);
