/*
 * SPDX-License-Identifier: Apache-2.0
 *
 * Copyright (c) 2020 Martin Jäger / Libre Solar
 */

#include <kernel.h>
#include <device.h>
#include <init.h>
#include <drivers/pinmux.h>
#include <sys/sys_io.h>

#include <pinmux/stm32/pinmux_stm32.h>

// preliminary fix, as Zephyr doesn't yet include all possible pinmuxes
#ifndef STM32F3_PINMUX_FUNC_PB6_I2C1_SCL
#define STM32F3_PINMUX_FUNC_PB6_I2C1_SCL \
	(STM32_PINMUX_ALT_FUNC_1 | STM32_OPENDRAIN_PULLUP)
#endif
#ifndef STM32F3_PINMUX_FUNC_PB7_I2C1_SDA
#define STM32F3_PINMUX_FUNC_PB7_I2C1_SDA \
	(STM32_PINMUX_ALT_FUNC_1 | STM32_OPENDRAIN_PULLUP)
#endif

#define STM32F3_PINMUX_FUNC_PA8_TIM1_CH1 \
	(STM32_PINMUX_ALT_FUNC_2 | STM32_PUSHPULL_NOPULL)
#define STM32F3_PINMUX_FUNC_PB13_TIM1_CH1N \
	(STM32_PINMUX_ALT_FUNC_2 | STM32_PUSHPULL_NOPULL)

static const struct pin_config pinconf[] = {
	/* TIM1_CH1 and TIM1_CH1N */
	{STM32_PIN_PA8, STM32F3_PINMUX_FUNC_PA8_TIM1_CH1},
	{STM32_PIN_PB13, STM32F3_PINMUX_FUNC_PB13_TIM1_CH1N},
#if DT_NODE_HAS_STATUS(DT_NODELABEL(usart1), okay) && CONFIG_SERIAL
	{STM32_PIN_PA9, STM32F3_PINMUX_FUNC_PA9_USART1_TX},
	{STM32_PIN_PA10, STM32F3_PINMUX_FUNC_PA10_USART1_RX},
#endif /* CONFIG_SERIAL || UART_CONSOLE, usart1 */
#if DT_NODE_HAS_STATUS(DT_NODELABEL(usart2), okay) && CONFIG_SERIAL
	{STM32_PIN_PA2, STM32F3_PINMUX_FUNC_PA2_USART2_TX},
	{STM32_PIN_PA3, STM32F3_PINMUX_FUNC_PA3_USART2_RX},
#endif /* CONFIG_SERIAL, usart2 */
#if DT_NODE_HAS_STATUS(DT_NODELABEL(i2c1), okay) && CONFIG_I2C
	{STM32_PIN_PB6, STM32F3_PINMUX_FUNC_PB6_I2C1_SCL},
	{STM32_PIN_PB7, STM32F3_PINMUX_FUNC_PB7_I2C1_SDA},
#endif /* CONFIG_I2C, i2c1 */
//~ #if DT_NODE_HAS_STATUS(DT_NODELABEL(i2c1), okay) && CONFIG_I2C
	//~ {STM32_PIN_PB10, STM32F3_PINMUX_FUNC_PB10_I2C2_SCL},
	//~ {STM32_PIN_PB11, STM32F3_PINMUX_FUNC_PB11_I2C2_SDA},
//~ #endif /* CONFIG_I2C, i2c1 */
#if DT_NODE_HAS_STATUS(DT_NODELABEL(spi1), okay) && CONFIG_SPI
	{STM32_PIN_PB3, STM32F3_PINMUX_FUNC_PB3_SPI1_SCK},
	{STM32_PIN_PB4, STM32F3_PINMUX_FUNC_PB4_SPI1_MISO},
	{STM32_PIN_PB5, STM32F3_PINMUX_FUNC_PB5_SPI1_MOSI},
#endif /* CONFIG_SPI, spi1 */
//~ #if DT_NODE_HAS_STATUS(DT_NODELABEL(can1), okay) && CONFIG_CAN
	//~ {STM32_PIN_PB8, STM32F3_PINMUX_FUNC_PB8_CAN_RX},
	//~ {STM32_PIN_PB9, STM32F3_PINMUX_FUNC_PB9_CAN_TX},
//~ #endif /* CONFIG_CAN, can1 */
};

static int pinmux_stm32_init(const struct device *port)
{
	ARG_UNUSED(port);

	stm32_setup_pins(pinconf, ARRAY_SIZE(pinconf));

	return 0;
}

SYS_INIT(pinmux_stm32_init, PRE_KERNEL_1,
	 CONFIG_PINMUX_STM32_DEVICE_INITIALIZATION_PRIORITY);
