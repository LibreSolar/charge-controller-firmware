...
{
    init_structs_buck();
    dcdc_port_lv.bus->voltage = dcdc.ls_voltage_min - 0.5;
    TEST_ASSERT_EQUAL(0, dcdc.check_start_conditions());
}
...
{
    init_structs_buck();
    dcdc_port_lv.bus->voltage = bat_conf.voltage_absolute_min - 0.1;
    TEST_ASSERT_EQUAL(0, dcdc.check_start_conditions());
}
...
{
    init_structs_buck();
    dcdc_port_lv.bus->voltage = bat_conf.topping_voltage + 0.1;
    TEST_ASSERT_EQUAL(0, dcdc.check_start_conditions());
}
...
{
    init_structs_buck();
    dcdc_port_lv.chg_current_limit = 0;
    TEST_ASSERT_EQUAL(0, dcdc.check_start_conditions());
}
...
{
    init_structs_buck();
    dcdc_port_hv.bus->voltage = dcdc.hs_voltage_max + 1;
    TEST_ASSERT_EQUAL(0, dcdc.check_start_conditions());
}
...
{
    init_structs_boost();
    dcdc_port_hv.bus->voltage = bat_conf.voltage_absolute_min - 0.1;
    TEST_ASSERT_EQUAL(0, dcdc.check_start_conditions());
}
...
{
    init_structs_boost();
    dcdc_port_hv.bus->voltage = bat_conf.topping_voltage + 0.1;
    TEST_ASSERT_EQUAL(0, dcdc.check_start_conditions());
}
...
{
    init_structs_boost();
    dcdc_port_hv.chg_current_limit = 0;
    TEST_ASSERT_EQUAL(0, dcdc.check_start_conditions());
}
...
{
    init_structs_boost();
    dcdc_port_lv.bus->voltage = dcdc.ls_voltage_max + 1;
    TEST_ASSERT_EQUAL(0, dcdc.check_start_conditions());
}
...
void buck_derating_output_voltage_too_high()
{
    start_buck();
    float pwm_before = half_bridge_get_duty_cycle();
    dcdc_port_lv.bus->voltage = dcdc_port_lv.bus->chg_voltage_target + 0.1;
    dcdc.control();
    float pwm_after = half_bridge_get_duty_cycle();
...
void buck_derating_output_current_too_high()
{
    start_buck();
    float pwm_before = half_bridge_get_duty_cycle();
    dcdc_port_lv.current = dcdc_port_lv.chg_current_limit + 0.1;
    dcdc.control();
    float pwm_after = half_bridge_get_duty_cycle();
    TEST_ASSERT(pwm_after < pwm_before);
...
void buck_derating_input_voltage_too_low()
{
    start_buck();
    float pwm_before = half_bridge_get_duty_cycle();
    dcdc_port_hv.bus->voltage = dcdc_port_hv.bus->dis_voltage_stop - 0.1;
    dcdc_port_lv.current = 0.2;
    dcdc.control();
    float pwm_after = half_bridge_get_duty_cycle();
    TEST_ASSERT(pwm_after < pwm_before);
...
void buck_derating_input_current_too_high()
{
    start_buck();
    float pwm_before = half_bridge_get_duty_cycle();
    dcdc_port_hv.current = dcdc_port_hv.dis_current_limit - 0.1;
    dcdc.control();
    float pwm_after = half_bridge_get_duty_cycle();
    TEST_ASSERT(pwm_after < pwm_before);
    TEST_ASSERT_EQUAL(DCDC_STATE_DERATING, dcdc.state);
...
void buck_stop_high_voltage_emergency()
{
    start_buck();
    float pwm_before = half_bridge_get_duty_cycle();
    dcdc_port_lv.bus->voltage = dcdc.ls_voltage_max + 0.1;
    dcdc.control();
    TEST_ASSERT(half_bridge_enabled() == false);
}
...
void buck_correct_mppt_operation()
{
    ...
    dcdc_port_lv.power = 5;
    dcdc.control();
    float pwm1 = half_bridge_get_duty_cycle();
    dcdc_port_lv.power = 7;
    dcdc.control();
    float pwm2 = half_bridge_get_duty_cycle();
    TEST_ASSERT(pwm2 > pwm1);

    dcdc_port_lv.power = 6;     // decrease power to make the direction turn around
    dcdc.control();
...
void boost_derating_output_voltage_too_high()
{
    start_boost();
    float pwm_before = half_bridge_get_duty_cycle();
    dcdc_port_hv.bus->voltage = dcdc_port_hv.bus->chg_voltage_target + 0.5;
    dcdc.control();
    float pwm_after = half_bridge_get_duty_cycle();
...
void boost_derating_output_current_too_high()
{
    start_boost();
    float pwm_before = half_bridge_get_duty_cycle();
    dcdc_port_hv.current = dcdc_port_hv.chg_current_limit + 0.1;
    dcdc.control();
    float pwm_after = half_bridge_get_duty_cycle();
    TEST_ASSERT(pwm_after > pwm_before);
}
...
void boost_derating_input_voltage_too_low()
{
    start_boost();
    float pwm_before = half_bridge_get_duty_cycle();
    dcdc_port_lv.bus->voltage = dcdc_port_lv.bus->dis_voltage_stop - 0.1;
    dcdc_port_hv.current = 0.2;
    dcdc.control();
    float pwm_after = half_bridge_get_duty_cycle();
    TEST_ASSERT(pwm_after > pwm_before);
}
...
void boost_derating_input_current_too_high()
{
    start_boost();
    float pwm_before = half_bridge_get_duty_cycle();
    dcdc_port_lv.current = dcdc_port_lv.dis_current_limit - 0.1;
    dcdc.control();
    float pwm_after = half_bridge_get_duty_cycle();
    TEST_ASSERT(pwm_after > pwm_before);
}
...
{
    start_boost();
    dcdc_port_hv.bus->voltage = dcdc.hs_voltage_max + 0.1;
    dcdc.control();
    TEST_ASSERT(half_bridge_enabled() == false);
}
...
void boost_correct_mppt_operation()
{
    ...
    dcdc_port_hv.power = 5;
    dcdc.control();
    float pwm1 = half_bridge_get_duty_cycle();
    dcdc_port_hv.power = 7;
    dcdc.control();
    float pwm2 = half_bridge_get_duty_cycle();
    TEST_ASSERT(pwm2 < pwm1);

    dcdc_port_hv.power = 6;     // decrease power to make the direction turn around
    dcdc.control();
    float pwm3 = half_bridge_get_duty_cycle();
    TEST_ASSERT(pwm3 > pwm2);
}
